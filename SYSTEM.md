...
| `dashes` | **Added** lines (git diff) in changed `.md` / `.txt` / `.html` contain a decorative U+2014 or any U+2013, except `VERIFY_BEFORE_DONE.md` and `punctuation-lists.md` (those files contain intentional examples). Space-dash-space list/apposition fails. Ironic cut-off U+2014 with letters on both sides and no surrounding spaces is allowed. Untracked files are scanned in full. Historical dashes already on HEAD do not fail a later edit |
